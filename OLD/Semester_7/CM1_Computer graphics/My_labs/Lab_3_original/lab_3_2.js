...
const ballSpeed = [];
const rotAxis = [];
const omega = [];   //это угловые скорости всех шаров (для вращения шара через rotMat)
...
function roll(t,dt){
	for(let i=0; i<balls.length; i++){
		//вот этот код прибавляет к текущей позиции величину равную скорости умножить на dt
		//где dt - сколько времени прошло с прошлого кадра
		//то есть классическая формула s = v*dt
		//s - вычисляемое перемещение, v = ballSpeed, ну и dt - сколько времени двигались с 
		//прошлого кадра
		//после любого такого перемещения в сторону бортика можно получить координату по одной из
		//осей в бортике
		//мы меняем в таком случае скорость на противоположную, но судя по всему за следующий кадр 
		//перемещение происходит не достаточное, что-бы покинуть бортик и мы там застрянем на некоторое время
		//будем лечить тем, что когда определили, что шар по какой-то координате в бортике - мы его 
		//будем перемещать из бортика - все будет выглядеть натурально
		//не совсем, они перемещаются в сторону бортика и могут попасть в него
		//представь себ, что в какой-то кадр шар оказался очень очень близко к бортику, за следующий кадр
		//он переместится на длинну равную части радиуса шара, тем большую, чем больше скорость и 
		//больше оказалось dt (эта величина не постоянная, бывает с прошлого кадра прошло 2мс, а бывает 4мс)
		//потому шар может легко оказаться в бортике, если не корректировать позицию, что мы и сделаем
		
		//здесь написано ballSpeed[i].clone() потому, что нам нужно умножить вектор скорости на длительность
		//но в JS.Three метод multiplyScalar изменит сам вектор скорости
		//мы же не хотим менять скорость, мы хотим вычислить на какое расстояние сместится шар, движущийся 
		//со скоростью ballSpeed за время dt
		//потому приходится делать копию вектора скорости для каждого шара каждый раз при таком расчете
		//это идиотизм, в c# так не делаю, там вообще такая операция выглядела бы 
		//movement = speed * dt;
		//на самом дела причина в том, как реализовано это в библиотеке Three
		//хотя возможно и сам кривой язык JS внес свою лепту
		pos[i].add(ballSpeed[i].clone().multiplyScalar(dt));
		//дело в том, что в 3Д матрица трансформации МОЖЕТ делать 3 действия:
		//перемещать объект
		//крутить объект
		//+ еще и изменять его размер
		const transMat = new THREE.Matrix4();
		//метод makeTranslation перемещает
		transMat.makeTranslation(pos[i].x, pos[i].y, pos[i].z);
		//тебя сбивает с толку length
		//на самом деле это снова "прелесть" реализации библиотеки
		//в данном случае - это длинна вектора скорости
		//ибо угловая скорость вычисляется не как вектор, а как абсолютная величина
		//потому и мутят дальше еще с rotMat, чтобы вращать объект с такой угловой скоростью
		//omega - показывает скорость радиального(углового) вращения, но само вращение идет через rotMat
		omega[i] = ballSpeed[i].length()/ballRadius;
		const rotMat = new THREE.Matrix4();
		//метод makeRotationAxis - один из методов вращения
		//ну и еще, еслибы мы захотели изменение размеров шаров - можно было бы и скейлинг добавить
		rotMat.makeRotationAxis(rotAxis[i], omega[i]*t);
		//здесь мы к нашей матрице применяем перемещение
