...
var mat = new THREE.MeshPhongMaterial({color: "white", wireframe:true, wireframeLinewidth:2} );
//speed
...
function render() {
  ...
  var posInCam = obj.position.clone();
  posInCam.applyMatrix4(camera.matrixWorldInverse);//position of ball in camera space - it is negated original position, in this case (0,0,-5)
  var zc = Math.abs(posInCam.z);
  var hc = 2*zc*Math.tan(Math.PI/180 * camera.fov/2); //make in radians
  var wc = camera.aspect * hc;
  if(Math.abs(posInCam.x) >= wc/2 - radius){
	  v.x *= -1;
  }
