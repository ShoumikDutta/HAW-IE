...
#pragma DATA_ALIGN(FFT_buf, 2*sizeof(float))	//align input FFT_buf on boundary
//------------ prototypes -------------------------------------------------------------
...
// generate signel with 8 sampels, MUST BE in normal order !!
	for (i=0;i<N_FFT;i++) {
		FFT_buf[2*i] = (float) ( 8192 * cos(2*PI*2*i/N_FFT) );//+ 
		FFT_buf[2*i+1] = 0;
	}
...
// calculate FFT via assembler routine
	cfftr2_dit( FFT_buf, w, N_FFT);
	j=0;
    for (i=0;i<2*N_FFT;i+=2) 
    {
		indx = 2*index[j];
	 	out_buf[j]= (	(FFT_buf[indx]   * FFT_buf[indx] + 
	 					 FFT_buf[indx+1] * FFT_buf[indx+1])) / float_scale;
	 	j++;
	}
