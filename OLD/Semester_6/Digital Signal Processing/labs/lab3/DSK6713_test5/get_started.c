...
//  external beim DSK-Board, hier zu deklarieren für Simulator:
	...
	short int buff_delay[N_FIR_coeffs] = {0};
...
// buffer full ??
	for(i = N_FIR_coeffs -1; i>0; i--){
		buff_delay[i] = buff_delay[i-1];
	}

	buff_delay[0] = AIC23_data.channel[RIGHT];

	FIR_accu32 = 0;					// set to 0 to prevent old data
	for (i=0; i<N_FIR_coeffs; i++){
		FIR_accu32 += h[i]*buff_delay[i];
	}


	FIR_out_BP = (short)(FIR_accu32 >> 15);
	AIC23_data.channel[RIGHT] = FIR_out_BP;
	AIC23_data.channel[LEFT] = FIR_out_BP;
	count_INT++;
    if (count_INT >= BUFLEN)
    	count_INT = 0;
...
//	}
//
//	buff_delay[0] = AIC23_data.channel[LEFT];
//
//	FIR_accu32 = 0;					// set to 0 to prevent old data
//	for (i=0; i<N_FIR_coeffs; i++){
//		FIR_accu32 += h[i]*buff_delay[i];
