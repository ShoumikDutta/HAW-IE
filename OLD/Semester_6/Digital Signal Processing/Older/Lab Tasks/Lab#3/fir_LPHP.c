...
//	inBuf_R[count_INT] = AIC23_data.channel[RIGHT];
		...
		for (i = 0; i < N_FIR_LP_coeffs; i++) {
			FIR_accu32 += b_FIR_LP[i] * delays[i];
		}
		FIR_out_LP = (short) (FIR_accu32 >> 15);
		FIR_accu32 = 0;

		AIC23_data.channel[LEFT] = FIR_out_LP;
