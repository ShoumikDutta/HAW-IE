...
//-------------------------------------------------------------------------
//Give from the Starter File
//#define N_FFT 8

//Self Defined According to the prep
#define N_FFT 64
#define LEFT 1
#define RIGHT 0
...
interrupt void intser_McBSP1() {
	...
	if (pFFT_buf < N_FFT) {
		FFT_buf[2 * pFFT_buf] = AIC23_data.channel[LEFT]
				+ AIC23_data.channel[RIGHT];	//add LADC and RADC

		FFT_buf[2 * pFFT_buf+1] = 0;
		pFFT_buf++; //increment the buffer pointer
	}
...
// input buffer FFT_buf[2*NFFT] should be cleared first!
	for (i = 0; i < 2 * N_FFT; i++) {
		FFT_buf[i] = 0.0;                 // clear the buffer
	}
...
//		indx = 2 * index[j];
//		out_buf[j] = ((x1[indx] * x1[indx] + x1[indx + 1] * x1[indx + 1]))
//				/ float_scale;
//		j++;
//	}
...
// put your code for buffer handling of spectrum analyser into while(1)-loop
	while (1) {
		if (pFFT_buf == N_FFT) {
			pFFT_buf=0;
			cfftr2_dit(FFT_buf, w, N_FFT); // calculate the FFT
			j = 0;
			for (i = 0; i < 2 * N_FFT; i += 2) {
				indx = 2 * index[j];
				out_buf[j] =
						((FFT_buf[indx] * FFT_buf[indx] + FFT_buf[indx + 1] * FFT_buf[indx + 1]))
								/ float_scale;
				j++;
			}
			//calculate the amp ratio
			amp_ratio = sqrt(FFT_buf[2*index[1]]*FFT_buf[2*index[1]] + FFT_buf[2*index[1]+1]*FFT_buf[2*index[1]+1]) / sqrt(FFT_buf[2*index[3]]*FFT_buf[2*index[3]]+FFT_buf[2*index[3]+1]*FFT_buf[2*index[3]+1]);
			// calculate the phase diff
			phase_diff = atan(FFT_buf[2*index[1]+1]/FFT_buf[2*index[1]] ) - atan(FFT_buf[2*index[3]+1]/FFT_buf[2*index[3]] );
		}
