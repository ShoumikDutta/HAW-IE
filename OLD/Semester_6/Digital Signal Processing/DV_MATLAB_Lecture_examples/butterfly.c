...
void butterfly(short *p_xa, short *p_xb, short w1, short w2){
	...
	int xout_r2_temp;
	int xout_i2_temp;
// upper node, real part
	xout_r1_temp = (  *p_xa    << 15 ) - *p_xb * w1 - *(p_xb+1) * w2 ;
// upper node, imaginary part
	xout_i1_temp = (  *(p_xa+1)<< 15 ) + *p_xb * w2 - *(p_xb+1) * w1 ; 
// lower node, real part
	xout_r2_temp = (  *p_xa    << 15 ) + *p_xb * w1 + *(p_xb+1) * w2 ; 
// lower node, imaginary part
	xout_i2_temp = (  *(p_xa+1)<< 15 ) - *p_xb * w2 + *(p_xb+1) * w1 ; 

	*p_xa     = (short) (xout_r1_temp>>15); //write four results back IN-PLACE
	*(p_xa+1) = (short) (xout_i1_temp>>15);
	*p_xb     = (short) (xout_r2_temp>>15);
	*(p_xb+1) = (short) (xout_i2_temp>>15);
}