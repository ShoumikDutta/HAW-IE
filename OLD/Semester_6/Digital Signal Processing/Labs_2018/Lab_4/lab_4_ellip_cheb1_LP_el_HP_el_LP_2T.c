...
short int x_n_el, x_n_cheb1, x_n_el_HP, x_n_el_2T;
short int i, y_n_el, y_n_cheb1, y_n_el_HP, y_n_el_2T;
...
int T1_el_HP[2], T2_el_HP[2], accu32_el_HP; //2 - because we have two second-order sections (sos)
int T1_el_2T[2], T2_el_2T[2], T3_el_2T[2], T4_el_2T[2], accu32_el_2T; //2 - because we have two second-order sections (sos)
//for el_2T we got in MATLAB a 4th-order IIR filter, so we have to split it into 2 sos
...
void cheb1_LP()
{
	for(i = 0; i < N + 1; i++) //N+1 - because we have 3 sections
	{
...
void ellip_HP()
{
	for(i = 0; i < N; i++)
	{
		accu32_el_HP = num_IIR_HP_EL[3*i+0] * x_n_el_HP + T1_el_HP[i];
		y_n_el_HP = (short) (accu32_el_HP >> 15);
		T1_el_HP[i] = num_IIR_HP_EL[3*i+1] * x_n_el_HP - den_IIR_HP_EL[2*i+0]*y_n_el_HP + T2_el_HP[i];
		T2_el_HP[i] = num_IIR_HP_EL[3*i+2] * x_n_el_HP - den_IIR_HP_EL[2*i+1]*y_n_el_HP;
		x_n_el_HP = y_n_el_HP;
	}
	AIC23_data.channel[LEFT] = y_n_el_HP;
}
...
void ellip_LP_2T()
{
	for(i = 0; i < N; i++)
	{
		accu32_el_2T = num_IIR_LP_EL_2T[5*i+0] * x_n_el_2T + T1_el_2T[i];
		y_n_el_2T = (short) (accu32_el_2T >> 15);
		T1_el_2T[i] = num_IIR_LP_EL_2T[5*i+1] * x_n_el_2T - den_IIR_LP_EL_2T[4*i+0]*y_n_el_2T + T2_el_2T[i];
		T2_el_2T[i] = num_IIR_LP_EL_2T[5*i+2] * x_n_el_2T - den_IIR_LP_EL_2T[4*i+1]*y_n_el_2T + T3_el_2T[i];
		T3_el_2T[i] = num_IIR_LP_EL_2T[5*i+3] * x_n_el_2T - den_IIR_LP_EL_2T[4*i+2]*y_n_el_2T + + T4_el_2T[i];
		T4_el_2T[i] = num_IIR_LP_EL_2T[5*i+4] * x_n_el_2T - den_IIR_LP_EL_2T[4*i+3]*y_n_el_2T;
		x_n_el_2T = y_n_el_2T;
	}
	AIC23_data.channel[LEFT] = y_n_el_2T;
}
...
interrupt void intser_McBSP1()
{
	...
	x_n_el_HP = AIC23_data.channel[LEFT];
	x_n_el_2T = AIC23_data.channel[LEFT];
